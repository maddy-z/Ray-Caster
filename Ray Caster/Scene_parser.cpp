...
void SceneParser::initialize()
{
	...
	group = NULL;
	camera = NULL;
	background_color = Vec3f(0.5f, 0.5f, 0.5f);
	// lights = NULL;
	// num_lights = 0;
	current_object_color = Vec3f(1.0f, 1.0f, 1.0f);
	file = NULL;
}
...
Group* SceneParser::parseTriangleMesh() 
{
	...
	while (1) 
	{
		...
		if (c == EOF) { break;} 
			...
			// CALLING ASSIGNMENT 2 CODE
			// create a new triangle object and return it
			Triangle *t = new Triangle(verts[f0-1], verts[f1-1], verts[f2-1], current_object_color);
